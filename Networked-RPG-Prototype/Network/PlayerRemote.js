...
class State extends System.Object {
    ...
    function FixedUpdate() {
        ...
        simulatePhysics = (localPlayer && Vector3.Distance(localPlayer.GetComponent.<Rigidbody>().position, GetComponent.<Rigidbody>().position) < 30);
        ...
        GetComponent.<Rigidbody>().isKinematic = !simulatePhysics;
        GetComponent.<Rigidbody>().interpolation = (simulatePhysics ? RigidbodyInterpolation.Interpolate : RigidbodyInterpolation.None);
        ...
        if (states[0].t > interpolationTime) {												// Target playback time should be present in the buffer
            ...
            for (i=0; i<stateCount; i++) {													// Go through buffer and find correct state to play back
                if (states[i] && (states[i].t <= interpolationTime || i == stateCount-1)) {
                    ...
                    if(simulatePhysics) {
                        GetComponent.<Rigidbody>().position = Vector3.Lerp(GetComponent.<Rigidbody>().position, Vector3.Lerp(lhs.p, rhs.p, t), physInterp);
                        GetComponent.<Rigidbody>().rotation = Quaternion.Slerp(GetComponent.<Rigidbody>().rotation, Quaternion.Slerp(lhs.r, rhs.r, t), physInterp);
                        GetComponent.<Rigidbody>().velocity = ((rhs.p - states[i + 1].p) / (rhs.t - states[i + 1].t));
                    }
                    else {
                        GetComponent.<Rigidbody>().position = Vector3.Lerp(lhs.p, rhs.p, t);
                        GetComponent.<Rigidbody>().rotation = Quaternion.Slerp(lhs.r, rhs.r, t);
                    }
                    isResponding = true;
                    netCode = "";
                    return;
            ...
            if (extrapolationLength < 1 && states[0] && states[1]) {
                if(!simulatePhysics) {
                    GetComponent.<Rigidbody>().position = states[0].p + (((states[0].p - states[1].p) / (states[0].t - states[1].t)) * extrapolationLength);
                    GetComponent.<Rigidbody>().rotation = states[0].r;
                }
                isResponding = true;
        ...
        if(simulatePhysics && states[0].t > states[2].t) GetComponent.<Rigidbody>().velocity = ((states[0].p - states[2].p) / (states[0].t - states[2].t));
    }
